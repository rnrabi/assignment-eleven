...
const FoodPurchase = () => {
    ...
    if (user.email === purchase.addBy.email) {
        Swal.fire("Don't purchase it because of owner");
        return <Navigate to={`/detail/${purchase._id}`}></Navigate>
    }

    if (purchase.quantity == 0) {
        Swal.fire("This item is not available");
    }
            ...
            <section className="max-w-4xl p-6 mx-auto bg-white rounded-md shadow-md dark:bg-gray-800">
                ...
                <form onSubmit={handlePurchase}>
                    ...
                    <div className="flex justify-end mt-6">
                        <button disabled={purchase.quantity == 0} className=" px-8 py-2.5 leading-5 text-white transition-colors duration-300 transform bg-gray-700 rounded-md hover:bg-gray-600 focus:outline-none focus:bg-gray-600">Purchase now</button>
                    </div>
                </form>
            </section>
        </div>
